...
namespace SoundCenSe.fmodInternal
{
    public class fmodPlayer : IDisposable, IPlayerManager
    {
        ...
        {
            SoundSoundFile sf = new SoundSoundFile(sound, sound.GetRandomSoundFile());
        ...
        {
            fmodChannelSound fcs = FmodChannelPool.Instance.GetSingleChannel(channel);
            fcs.Channel.stop();
        }
        ...
        private void LoopSound(object sender, RestartSoundLoopEventArgs restartSoundLoopEventArgs)
        {
            ...
            SoundSoundFile ssf = new SoundSoundFile(fmc.SoundSoundFile.Sound, fmc.SoundSoundFile.SoundFile);

            if (ssf.Sound.SoundFiles.Count > 1)
            {
                SoundFile sf = ssf.SoundFile;
                while (sf == ssf.SoundFile)
                {
                    sf = ssf.Sound.GetRandomSoundFile();
                }
                ssf.SoundFile = sf;
        ...
        public void PlaySound(SoundSoundFile sf, string channel, float volume = 1.0f, bool mute = false)
        {
            ...
            if (sf.SoundFile == null)
            {
                sf.SoundFile = sf.Sound.GetRandomSoundFile();
            }
            if (sf.SoundFile == null)
            {
                return;
        ...
        {
            channelGroups[channel].setVolume(volume);
        }
    }
}